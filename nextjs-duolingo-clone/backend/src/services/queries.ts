...
export const getUnits = async (userId: string | null) => {
  if (!userId) {
    return [];
  }
...
export const getCourseProgress = async (userId: string | null) => {
  if (!userId) {
    return null;
  }
...
export const getLesson = async (userId: string | null, id?: number) => {
  if (!userId) {
    return null;
  }

  const courseProgress = await getCourseProgress(userId);
  const lessonId = id ?? courseProgress?.activeLessonId ?? undefined;
...
export const getLessonPercentage = async (userId: string | null) => {
  if (!userId) {
    return 0;
  }
